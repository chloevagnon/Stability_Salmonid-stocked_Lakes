...
############################################### Binary_Mat ###########################################################
# Transform the parameters into a binary interaction matrix basd on Gravel et al. 2013
...
Rel_Ab_Mat<-function(Nodes_name,Nodes_Category,Nodes_Density,Bmat){
  ...
  # Rescaling is diferent if the interaction is between one fish and the rest or one inv/zoo and phyto as units
...
########################################## Make_Bern #######################################################
# Create multiple binary matrices from on Bernouilli trials based on a ma trix of interaction probability
...
# If Method != "Random":
#    a jacobian binary matrix with interaction strength drawn randomly following a half normal distribution, symetrically
...
#
#Returns :
# a jacobian binary matrix with interaction strength scaled to predator/prey body size ratios accouting a different scaling
...
Scale_JmatBS <- function(Jmat, Nodes, Node_Category,Node_Size, RangeVert,RangeInvert){
  ...
  if(length(lower_tri_str_index[Node_Category[lower_tri_str_index[,2]]=="Fish",2])!=0){
